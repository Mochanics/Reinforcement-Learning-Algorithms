#==================================================================================================
#Q-Learning code by Mochanics based upon code snippets from the following people:
#Abid Ali Awan: https://www.datacamp.com/tutorial/introduction-q-learning-beginner-tutorial
#==================================================================================================
...
GAME = "FrozenLake-v1"
REWARD_SCHEDULE = (5, -5, -0.01)

EPISODES = 15000 #Total number of learning episodes
        ...
        if recording_mode == True:
            with warnings.catch_warnings(action="ignore"):
                self.env = RecordVideo(
                    self.env,
                    video_folder="./q-learning/recordings", # Folder to save videos
...
def train_agent():
    ...
    print("Time: ", time)
    with open( './q-learning/timeProfiling.log', 'w' ) as f:
        f.write(str(time))
    ...
    if PLOTTING == True:
        ...
        np.savetxt('./q-learning/training_graph_data.csv', np.column_stack([episodes_num, rewards]), delimiter=",", header="Episodes, Rewards", fmt='%s')
...
def evaluate_agent(Qtable):
    ...
    #Saving plot data to a csv file
    np.savetxt('./q-learning/evaluating_graph_data.csv', np.column_stack([episodes_num, rewards, steps]), delimiter=",", header="Episodes, Rewards, Steps", fmt='%s')
...
class Profiler(Thread):
    ...
    def run(self):
        ...
        with open('./q-learning/memoryProfiling.log', 'w') as f:
            for value in values:
                f.write(str(value) + "\n")
...
if __name__ == "__main__":      
    ...
    if TRAINING_MODE: #If in training mode
        ...
        if SPEED_PROFILING == True:
            pr.disable()
            with open( './q-learning/profilingResults.log', 'w' ) as f:
                pstats.Stats(pr, stream=f).strip_dirs().sort_stats("cumulative").print_stats()
    
        np.savetxt("./q-learning/q-table.txt", Qtable)
        print(Qtable)
    
    else: #If not in training mode
        Qtable = np.loadtxt("./q-learning/q-table.txt")
