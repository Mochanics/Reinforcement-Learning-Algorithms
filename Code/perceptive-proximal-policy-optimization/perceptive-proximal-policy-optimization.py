#==============================================================================================================
#Perceptive Proximal Policy Optimization code by Mochanics based upon code snippets from the following people:
...
ROLLOUT_STEPS = 1024 #Current number of time steps to be recorded in a rollout buffer.
...
CURRICULUM_ENTROPY = 0.1 #Entropy boost/rest. It is the value the entropy variable gets when the curriculum episode count is reached (CURRICULUM_START). This only applies when GENERATE_MAP_PER_EPISODE is true.
ENTROPY_COEF_MIN = 0.05 #Minimum entropy at the end of training.
ENTROPY_DECAY = 0.9995  #How fast the entropy decreases.
DETERMINISTIC = False #Whether the agent selects actions at random based on probabilities of which one is the best (non-deterministic) or always takes the best action (determinsitic) during testing.
MAX_EPISODES = 5000 #The number of training episodes.
...
CURRICULUM_START = 1500 #Start of when maps are being randomly generated. The probability of them being randomly generated increases until CURRICULUM_END. This only applies when GENERATE_MAP_PER_EPISODE is true.
CURRICULUM_END = 2500 #When ALL maps are randomly generated. This only applies when GENERATE_MAP_PER_EPISODE is true.
...
#------State Preprocessing--------
#=================================
def get_obs_dim(env): #Returns the observation vector dimension for the given environment.
    grid_size = env.unwrapped.nrow
    return 4 + grid_size * grid_size
...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_dim):
        ...
        self.net = nn.Sequential(         
            nn.Linear(obs_dim, 128), #Input layer to 1st hidden layer (observation space)
            nn.LayerNorm(128), #To prevent saturated neurons
            ...
            nn.Linear(128, action_dim), #2nd hidden layer to output layer (action space)
        )
...
class Critic(nn.Module):
    def __init__(self, obs_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.LayerNorm(128), #To prevent saturated neurons
            nn.ReLU(inplace=True), #Same ReLU activation function between 1st and 2nd layers
            nn.Linear(128, 1), #2nd hidden layer to output layer (action space)
        )
...
def train():
    ...
    obs_dim = get_obs_dim(env)  #obs_dim is now the rich state dimension for FrozenLake (2 + grid_size^2)
    action_dim = env.action_space.n #Size of the action space

    actor = Actor(obs_dim, action_dim).to(DEVICE) #Creating the actor (policy) network
    ...
    for episode in range(MAX_EPISODES):
        ...
        frac = max(0.1, 1.0 - episode / MAX_EPISODES)
...
def evaluate(model, episodes):
    ...
    for episode in range(episodes):
        ...
        if GENERATE_MAP_PER_EPISODE: #Generates a new map per evaluating episode
            env.close()
            env = generate_new_map("rgb_array", GENERATE_MAP_PER_EPISODE)
...
if __name__ == "__main__": 
    ...
    if TRAINING_MODE: #If in training mode
        ...
        loaded_model = Actor(obs_dim=obs_dim, action_dim=env.action_space.n).to(DEVICE)
        env.close()
        #--------------------------------------------------------------------------------------------------
